...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $sql2 = "delete FROM `Parents` WHERE Parent_ID ='$id';";
    $re = mysqli_query($link,$sql2);
    if($re){
       echo "<script>javascript:alert('yes!');location.href='delParent.php';</script>";
    }else{
       echo "<script>javascript:alert('no!');location.href='delParent.php';</script>";
    }
...
<h3>See all Parents</h3>
		...
		<table>
			...
			<tr>
                ...
                <th>{$row['Address']}</th>
                <th>{$row['Email']}</th>
                <th><a href='delParent.php?id=".$row['Parent_ID']."'>del</a></th>
